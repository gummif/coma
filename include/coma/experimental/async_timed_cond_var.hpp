...
public:
	...
	COMA_NODISCARD boost::asio::awaitable<void> async_wait(Predicate pred)
	{
		while (!pred())
			co_await async_wait();
	}
	...
	boost::asio::awaitable<cv_status> async_wait_until(time_point endtime)
	{
		...
		co_await async_wait();
		co_return clock_type::now() >= endtime ? cv_status::timeout : cv_status::no_timeout;
	}

	template<class Predicate>
	COMA_NODISCARD boost::asio::awaitable<bool> async_wait_until(time_point endtime,
																 Predicate pred)
	{
		while (!pred())
		{
	...
	template<class Predicate>
	COMA_NODISCARD boost::asio::awaitable<bool> async_wait_for(duration dur, Predicate pred)
	{
...
private:
	timer m_timer;
	std::multiset<time_point> m_times; // TODO use sorted vector of time_point
};
