...
TEST_CASE("stranded simple invoke", "[stranded]")
{
	...
	coma::stranded<std::vector<int>, typename boost::asio::io_context::executor_type> st{
		ctx.get_executor()};
...
{
	boost::asio::io_context ctx;
	coma::stranded<boost::asio::steady_timer, typename boost::asio::io_context::executor_type>
		st{ctx.get_executor(), std::in_place, ctx.get_executor()};
	...
	boost::asio::co_spawn(
		...
		[&]() -> boost::asio::awaitable<void> {
			...
			co_await st.invoke(
				[](auto& v) { return v.async_wait(boost::asio::use_awaitable); });
		},
		boost::asio::detached);
